...
> 💡 전투지도/토큰 이미지는 **Firebase Storage를 쓰지 않습니다.** Storage는 버킷을 새로
> 만들 때 Blaze(종량제) 요금제 전환(결제 수단 등록)이 필요할 수 있어서, 그 대신 압축한
> 이미지를 Realtime Database에 그대로 저장하는 방식을 씁니다. 위 3번 규칙만 설정하면
> 지도 공유까지 카드 등록 없이 바로 됩니다.
...
- `location_lookup`(지도 이미지를 직접 봐야 하는 드문 경우)은 협동 세션에서는
  ...
  안내 메시지가 표시됩니다.
- 전투지도(배경 이미지/토큰 이미지)는 카드 등록이 필요 없도록 **Firebase Storage를 쓰지
  않고**, 솔로 모드와 동일하게 로컬에서 압축한 이미지를 그대로 Realtime Database의 방
  데이터(`mapState`)에 저장해요. 화면 확대/이동(줌·팬)은 각자 보는 화면의 개인 설정이라
  공유되지 않고, 그 외 지도/토큰 관련 변경은 전부 실시간으로 모두에게 동기화됩니다.
- 이미지는 업로드 시 자동으로 압축돼서(지도는 가로 최대 1200px, 토큰은 400px) 용량은
  크지 않지만, 그래도 base64로 통째로 저장/전송되는 방식이라 아주 여러 장을 자주 바꿔
  올리는 용도로는 안 맞을 수 있어요. 세션 하나에서 지도 몇 장 쓰는 일반적인 용도로는
  문제없습니다.

## 참고

무료 요금제(Spark 플랜) 기준 Firebase Realtime DB는 동시 연결 100개, 저장 용량 1GB,
월 다운로드 10GB까지 무료라서, 지인끼리 쓰는 용도로는 **카드 등록이나 비용 걱정 없이**
쓸 수 있어요 (이 프로젝트는 Storage를 쓰지 않으므로 Blaze 요금제 전환도 필요 없습니다).
